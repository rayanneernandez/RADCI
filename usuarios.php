...
// Estatísticas
$total = count($users);
$cidadaos = count(array_filter($users, fn($u) => intval($u['perfil']) === 1));
$adminsPublicos = count(array_filter($users, fn($u) => intval($u['perfil']) === 3));
...
<body class="bg-white min-h-screen">
    ...
    <header class="bg-green-700 text-white">
        <div class="container mx-auto px-6 py-5 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8 text-white/90">
                <a href="principal.php" class="hover:text-white">Início</a>
    ...
    async function fillFromCep(cepDigits) {
        try {
            const res = await fetch(`api_viacep.php?cep=${encodeURIComponent(cepDigits)}`);
            if (!res.ok) return;
